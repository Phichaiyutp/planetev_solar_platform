...
import {
  ...
  ConsumpionToday,
  TotalCo2,
} from "@/components/dashboard/overall";
...
export default async function Home() {
  ...
  return (
    <div className=" m-6 grid grid-cols-2 lg:grid-cols-8 gap-4">
      <label
        htmlFor="my-drawer-2"
        className="btn btn-primary btn-xs drawer-button col-span-2 lg:hidden"
      >
        =
      </label>
      <p className="h-20 w-fit text-base-300 col-span-2 lg:col-span-6 lg:font-bold text-2xl xl:text-4xl 2xl:text-5xl">
        SOLAR SUMMARY DASHBOARD
      </p>
      <div className="dropdown dropdown-bottom dropdown-end h-20 text-lg hidden lg:flex lg:col-span-2 lg:text-xl 2xl:text-3xl">
        {user.username && (
          <p
            tabIndex={0}
            role="button"
            className="pt-6 pr-4 text-end w-full align-middle text-base-500 capitalize xl:pl-30"
          >
            {user.username}
          </p>
        )}
      ...
      <p className="w-auto pt-4  font-bold col-span-2 lg:col-span-8 lg:m-4 text-2xl xl:text-3xl 2xl:text-4xl">
        Total Energy
      </p>
