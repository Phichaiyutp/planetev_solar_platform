...
const get_stations = async () => {
  ...
  try {
    ...
    if (result.rows[0].yield_today) {
      result.rows[0].yield_today = parseFloat(
        result.rows[0].yield_today.toFixed(2)
      );
    } else {
      result.rows[0].yield_today = 0;
    }
    if (result.rows[0].total_yield) {
      result.rows[0].total_yield = parseFloat(
        result.rows[0].total_yield.toFixed(2)
      );
    } else {
      result.rows[0].total_yield = 0;
    }
    return result.rows[0];
  } finally {
    client.release();
...
const get_tou = async () => {
  ...
  try {
    ...
    if (result.rows[0].total_on_peak) {
      result.rows[0].total_on_peak = parseFloat(
        result.rows[0].total_on_peak.toFixed(2)
      );
    } else {
      result.rows[0].total_on_peak = 0;
    }
    if (result.rows[0].total_off_peak) {
      result.rows[0].total_off_peak = parseFloat(
        result.rows[0].total_off_peak.toFixed(2)
      );
    } else {
      result.rows[0].total_off_peak = 0;
    }
    if (result.rows[0].energy_overall) {
      result.rows[0].energy_overall = parseFloat(
        result.rows[0].energy_overall.toFixed(2)
      );
    } else {
      result.rows[0].energy_overall = 0;
    }
...
const get_billing = (
  yield_on_peak: number,
  yield_off_peak: number,
  config: any[]
) => {
  function execution(config: any[], valueName: any) {
    ...
    if (findConfig && findConfig.expression !== null) {
      const result = parseFloat(
        eval(findConfig.expression).toFixed(2)
      ) as number;
      return result;
    } else {
